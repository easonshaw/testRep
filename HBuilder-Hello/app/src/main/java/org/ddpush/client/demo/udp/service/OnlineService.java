	...
	public class MyUdpClient extends UDPClientBase {
		...
		public void onPushMessage(Message message) {
			...
			if(message.getCmd() == 32){// 0x20 自定义推送信息
				...
				// 推送开关
				boolean pushable = sp.getBoolean("pushable", true); // 默认(首次登陆)为开启
				if (pushable) {
					int intFlag = (int) (Math.random() * 10000);
				//	notifyUser(intFlag, map, isLooping);
					notifyUser(intFlag, map, false); // 关闭循环，只响一次
				}
				SendBroadcast(str);
			}
			setPkgsInfo();
...
//		n.setLatestEventInfo(this, data.get("fromname").toString(), data.get("content").toString(), pendingIntentClick);
		...
		n.icon = R.drawable.icon;
		n.when = System.currentTimeMillis();
		n.tickerText = data.get("fromname").toString() + "发来一条消息";
		notificationManager.notify(id, n);
...
//				player.setVolume(1f, 1f);// 音量调节，左右声道都为最大值
//
//				if (loopingflag) { // 是否循环
//					player.setLooping(true);// 开启单曲循环
//					player.start();
//				} else { // 其他情况响3次
//					soundCount = 2;
